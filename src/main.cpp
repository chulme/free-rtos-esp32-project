...
const Milliseconds TASK_2_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_2_RATE), SLOT_WIDTH); // 20s
const Milliseconds TASK_3_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_3_RATE), SLOT_WIDTH); // 100
const Milliseconds TASK_4_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_4_RATE), SLOT_WIDTH); // 4.16s
const Milliseconds TASK_5_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_5_RATE), SLOT_WIDTH); // 4.16s
const Milliseconds TASK_6_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_6_RATE), SLOT_WIDTH); // 10s
const Milliseconds TASK_7_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_7_RATE), SLOT_WIDTH); // 33.33s
const Milliseconds TASK_8_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_8_RATE), SLOT_WIDTH); // 33.33s
const Milliseconds TASK_9_PERIOD = roundToTheNearestMultiple(calculateCyclePeriod(TASK_9_RATE), SLOT_WIDTH); // 5s
...
const Milliseconds roundToTheNearestMultiple(const Milliseconds value_to_round, const Milliseconds multiple)
{
    ...
    Serial.printf("%f\t%f\t%f\n", value_to_round, multiple, static_cast<Milliseconds>(remainder));
    return static_cast<Milliseconds>(static_cast<int64_t>(round(value_to_round)) + static_cast<int64_t>(round(multiple)) - remainder);
}
